...
## Ш4. Поставщики и ловушка таймаута (1.25 дня) — этап 3 ТЗ — ✅ ГОТОВО
...
**Гейт (пройден):** критерии приёмки 4 и 5 доказаны на живых заглушках через
настоящий HTTP. Проверяется не только наше состояние, но и НЕЗАВИСИМЫЙ счётчик со
стороны поставщика: сколько кодов он реально выдал. Наши таблицы могут врать, его
журнал — нет. 129 тестов / 532 проверки, PHPStan level 9 чист.

Доказанные сценарии:
- поставщик выдал код и списал его, но ответ не дошёл → повтор ТЕМ ЖЕ `request_id`
  подбирает тот же код; у поставщика ровно одна выдача, ко второму не ходили;
- авторитетный отказ A (пустой остаток) → переход на B → ровно одна выдача;
- A недоступен (соединение не устанавливается) → это ДОКАЗАННОЕ отсутствие выдачи
  → переход на B → ровно одна выдача;
- **неразрешённая неизвестность → на B НЕ уходим**, заказ остаётся в `delivering`,
  открытая попытка одна, и вторую БД открыть не даст;
- `request_id` строится из (заказ, поставщик, эпоха) и не зависит от номера
  сетевой попытки.

**Найдено по ходу:** в тестах поставщиков отсутствовал `Queue::fake()`, поэтому приём
вебхука синхронно протягивал всю цепочку до выдачи, и явный вызов доставки получал
уже доставленный заказ — проверялся бы не тот путь. Ровно та же ловушка, что с
`env_file` в Ш3: тест зелёный, а проверяет не то, что заявлено.

---

## Ш5. Сверка, журнал, восстановление (1 день) — этап 4 ТЗ, бонус — ✅ ГОТОВО
...
**Гейт (пройден):** каждая подброшенная аномалия ловится, и — что важнее — сверка
МОЛЧИТ на здоровых данных. Сверка, которая шумит на штатной работе, бесполезна:
её перестают читать через неделю, и настоящая аномалия теряется среди ложных.
148 тестов / 599 проверок.

Реализовано: восемь детекторов, эндпоинт `GET /ops/reconciliation` под токеном
(409 при нездоровье, чтобы вешать прямо в мониторинг), команда `shop:reconcile --full`,
подметальщик `orders:sweep-stuck`, быстрая сверка ежеминутно и полная ночью.

Быстрой сверки в расписании изначально не было: пять детекторов денежных
расхождений срабатывали только по ручному запросу эндпоинта или раз в сутки.
Нашлось это не тестом, а `/health` на чистом клоне — он честно показал, что
сверка не запускалась ни разу. Теперь расписание проверяется `ScheduleTest`,
и частота прогонов связана тестом с порогом свежести в `/health`: разъехавшись,
эти два числа дают либо вечно красный health при исправной системе, либо
зелёный при остановившейся сверке.

Ожидание пополнения склада — **warning, а не critical**: заказ в `out_of_stock`
это пауза, и если считать её инцидентом, система будет красной ровно тогда, когда
работает штатно (критерий приёмки №6).

**Дважды по ходу сработала собственная защита, и оба раза это было доказательством:**
- тест пытался «состарить» заказ, переписав время в журнале, — не дал триггер
  неизменяемости журнала. Старить пришлось честно, создавая заказ в прошлом;
- тест пытался вернуть выданный заказ в ожидающий статус — не дал триггер
  необратимости финального состояния. Это и стало отдельной проверкой.

---

## Ш6. Каталог под нагрузкой (0.5 дня) — этап 5 ТЗ, бонус — ✅ ГОТОВО
...
**Гейт (пройден):** тест проверяет ПЛАН, а не время ответа. Время на пустой базе
всегда хорошее и ничего не доказывает.

Замер на 5012 товарах: `Index Only Scan`, `Heap Fetches: 0`, 4 страницы, 0.083 мс.

**Найдено по ходу:** первая версия покрывающего индекса включала `(sku, name, currency)`,
а горячий запрос выбирает ещё и `supply_mode`. Одной недостающей колонки хватило,
чтобы план стал обычным `Index Scan` — **27 страниц вместо 4**. «Покрывающий индекс» —
это не свойство индекса, а отношение между индексом и конкретным запросом, и
проверяется оно только планом. Ни один тест до этого этапа такого не ловил.

Плюс `VACUUM` в генераторе каталога: без заполненной карты видимости `Index Only Scan`
невозможен в принципе, и план деградирует независимо от индекса.

---

## Ш7. Сдача (0.5 дня) — ✅ ГОТОВО

README написан и содержит всё, что требует задание: запуск, прогон тестов, воспроизведение
гонок и отказа поставщика командами, таблицу «критерий приёмки → тест → индекс», записку
о ключевых решениях, раздел о масштабировании, честный список того, что осталось за рамками,
и фактическое время.

Порядок этапов изменён относительно плана сознательно: README сделан раньше бонусных
Ш5 и Ш6. Причина — работа должна быть сдаваемой в любой момент, а без README она
не сдаётся вовсе, тогда как без бонусных этапов сдаётся.
