...
1. **Заказ — список позиций, у каждой `quantity = 1`.** Допущение первого этапа «один SKU
   на заказ» снято вторым: он прямо требует заказ из нескольких товаров, каждый от своего
   поставщика. Товары живут в `order_items`; колонки `orders.product_id` и `orders.sku`
   остались снимком первой позиции ради кода первого этапа и уйдут вместе с ним.

   Итог заказа обязан равняться сумме позиций, и это держит **отложенный** триггер
   `order_items_total_matches`, а не проверка в сервисе. Отложенный — потому что позиции
   вставляются несколькими строками, и в середине операции сумма закономерно не сходится;
   немедленная проверка запретила бы создавать заказ из двух товаров вообще.

   Заказ без позиций отбивается тем же триггером — он висит и на `orders`. Это самый
   дорогой случай: заказ, за который заплатят, и в котором нечего выдавать.
...
3. Контракт заглушки расширен: `GET /issue/{request_id}` (probe), `POST /issue/{request_id}/seal`,
   `GET /issues?order_id=`, `PUT /admin/behavior`. Ответ на выдачу возвращает ещё и `sku`.
   Заглушки пишем мы — расширять контракт разрешено, и без `seal` критерий №5 первого этапа
   недостижим безопасно.

   **Граница защиты от нечестного поставщика названа прямо.** Дубль кода ловится независимо
   от поставщика: мы помним каждый код, который когда-либо получали, и это держит индекс
   `supplier_issued_codes_hash_uq`. «Код от другого товара» ловится сверкой ОБЪЯВЛЕННОГО
   поставщиком `sku` с запрошенным — то есть опирается на его декларацию. Полной защиты тут
   быть не может: что открывает чужая строка символов, снаружи не узнать. Это ограничение,
   а не недоделка, и оно записано здесь, чтобы никто не принял проверку за большее.
