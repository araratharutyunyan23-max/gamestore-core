...
final class SchemaContractTest extends TestCase
{
    ...
    public static function guaranteeIndexes(): array
    {
        return [
            ...
            'одна выдача на позицию' => ['deliveries_order_item_uq', 'каждый товар заказа выдаётся ровно один раз'],
            ...
            'одна открытая попытка на позицию' => ['delivery_attempts_item_open_uq', 'таймаут не приводит к уходу на второго поставщика'],
            'один успех на позицию' => ['delivery_attempts_item_success_uq', 'повтор после таймаута не создаёт вторую выдачу'],
